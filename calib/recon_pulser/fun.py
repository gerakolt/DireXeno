    ...
    while len(maxis)>0 and counter<1000:
        ...
        if np.amin(SPE)<np.amin(wf[left:right]):
            ...
            if I>50 and I<999-75:
                ...
                print('I is out the limits')
                spe=np.zeros(1000)
                spe[left:right]+=wf[left:right]
        ...
        # ffty=ffty/np.sum(ffty)
        # fftSPE=fftSPE/np.sum(fftSPE)
        # p=-np.sum(ffty*np.log((fftSPE+1e-10)/(ffty+1e-10))+ffty-fftSPE)
        ...
        #
        # plt.figure()
        # plt.plot(np.abs(ffty), '.-', label='{}'.format(np.round(np.sum(np.abs(ffty)[1:int(len(ffty)/2)])/np.sum(np.abs(ffty)[0:1]), decimals=3)))
        ...
        maxis=maxis[np.logical_and((wf[maxis]-wf[maxis-3])/wf[maxis]<dh3_cut, (wf[maxis]-wf[maxis+3])/wf[maxis]<dh3_cut)]
    return np.histogram(t, bins=np.arange(1001))[0], recon_wf, P
